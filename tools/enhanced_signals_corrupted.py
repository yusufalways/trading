...
def get_comprehensive_swing_signals(progress_callback=None, top_n: int = 15) -> Dict:
    """Comprehensive scan using ULTRA-FAST batch processing (same as ultra-fast but different name for dashboard compatibility)"""
    print("🚀 COMPREHENSIVE SCAN -> REDIRECTING TO ULTRA-FAST BATCH PROCESSING")
    # Simply call the ultra-fast function - no slow fallback methods
...
def get_ultra_fast_swing_signals(progress_callback=None, top_n: int = 15) -> Dict:
    """ULTRA-FAST market scan using optimized batch downloads and concurrent processing"""
    ...
    def batch_download_data(symbols: List[str], period: str = "1mo", batch_size: int = 30) -> Dict[str, pd.DataFrame]:
        """High-performance batch download with larger batches"""
        all_data = {}
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            try:
                # Use batch download with threading enabled
                ...
                # Fallback to individual downloads for this batch only
                for symbol in batch_symbols:
                    try:
                        ticker = yf.Ticker(symbol)
